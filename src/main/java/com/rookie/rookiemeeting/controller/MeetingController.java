...
public class MeetingController {
    ...
    public Result save(@RequestBody Meeting meeting) {
        Integer integer = meetingService.updateMeeting(meeting);
        if (integer==null){
            return Result.fail("系统错误，请稍后再试");
        }
        if (integer==-1){
            return Result.fail("当前会议室在该时间段已经被占用，请您换一个时间或更换会议室");
        }
        return Result.succ(null);
    }
    ...
    public Result addMeeting(@RequestBody Meeting meeting,
        ...
        Integer integer = meetingService.addMeeting(meeting, employeeid);
        if (integer==null){
            return Result.fail("系统错误，请稍后再设置");
        }
        if (integer==-1){
            return Result.fail("当前会议室在该时间段已经被占用，请您换一个时间或更换会议室");
        }
