...
public class MeetingServiceImpl extends ServiceImpl<MeetingMapper, Meeting> implements IMeetingService {
    ...
    public Integer addMeeting(Meeting meeting, Long employeeid) {
        ...
        LambdaQueryWrapper<Meeting> lambdaQueryChainWrapper = new LambdaQueryWrapper<>();
        lambdaQueryChainWrapper.eq(Meeting::getRoomid,meeting.getRoomid());
        List<Meeting> meetings = meetingMapper.selectList(lambdaQueryChainWrapper);
        for (Meeting meeting1 : meetings){
            if(meeting.getStarttime().compareTo(meeting1.getEndtime()) < 0 && meeting.getEndtime().compareTo(meeting1.getEndtime())>0){
                return -1;
            }
            if (meeting.getEndtime().compareTo(meeting1.getStarttime()) > 0 && meeting.getStarttime().compareTo(meeting1.getStarttime())<0){
                return -1;
            }
        }
